...
describe('Updater', () => {
  ...
  beforeEach(() => {
    ...
    data.forEach(torrent => {
      promises.push(
        models.Torrent.create(torrent).then(model => {
          const fprom = [];
  ...
  describe('oneToManyGetter.js', () => {
    it('should reject promise because parent unavailable', done => {
      const oneToManyGetter = oneToManyGetterFactory(
        models.File,
        'hash',
        'torrentHash',
      );
  ...
  describe('create', () => {
    ...
    it('should be able to retreive all model datas', done => {
      ...
      updater
        .init()
        .then(updater => {
          const torrentsHashes = [
            'aca11111456d83cf9a92048b93d06c4a3d873ce5',
            'aca11112456d83cf9a92048b93d06c4a3d873ce5',
          ];
  ...
  describe('update', () => {
    const filesGetter = oneToManyGetterFactory(
      models.File,
      'hash',
      'torrentHash',
    );
          ...
          files: {
            ...
            modelCreateCb: function(rawElement, rawParent) {
              return models.File.create(
                Object.assign({ torrentHash: rawParent.hash }, rawElement),
  ...
  afterEach(() => models.Torrent.truncate());
});
